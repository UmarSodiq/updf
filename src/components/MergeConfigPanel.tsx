...
import { Settings, FileText, ArrowRight, Trash2, GripVertical } from 'lucide-react';
...
export const MergeConfigPanel: React.FC<MergeConfigPanelProps> = ({ files, onStart, onCancel, onFilesUpdate }) => {
    ...
    >
      <div className="p-8">
        <h3 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2">
          ...
          Pengaturan Gabung PDF
        </h3>
        <p className="text-sm text-slate-500 mb-6 flex-1">
          Seret dan lepas untuk mengatur ulang file. File teratas akan menjadi halaman pertama dalam dokumen yang digabungkan.
        </p>
        ...
        <div className="mt-8 flex flex-col sm:flex-row gap-3">
          <button onClick={onCancel} className="flex-1 py-4 text-slate-600 font-semibold rounded-2xl border border-slate-200 hover:bg-slate-100 transition-colors">Batal</button>
          <button onClick={() => onStart(files)} className="flex-[2] py-4 bg-blue-600 text-white font-bold rounded-2xl shadow-sm hover:bg-blue-700 transition-colors flex items-center justify-center gap-2">
